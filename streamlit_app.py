...
def main():
    ...
    if len(st.session_state.button_sequence) <= 50:
        st.title(f"Frage: {st.session_state.total_button_clicks+1} / 50")
    ...
    st.write('Auswertung am Ende des Quizzes')
    ...
    if len(st.session_state.button_sequence) > 50:
        ...
        if st.button("Sequenz überprüfen"):
            ...
            st.title(f"Übereinstimmende Antworten: {match_count} von 50 Fragen")
